...
var action_opsGenie = process.env.TRIGGER_OPSGENIE || conf.TRIGGER_OPSGENIE || false;
...
// Setup action OpsGenie
var action_opsgenie_protocol = process.env.ACTION_OPSGENIE_PROTOCOL || conf.ACTION_OPSGENIE_PROTOCOL;
var action_opsgenie_host = process.env.ACTION_OPSGENIE_HOST || conf.ACTION_OPSGENIE_HOST;
var action_opsgenie_endpoint = process.env.ACTION_OPSGENIE_ENDPOINT || conf.ACTION_OPSGENIE_ENDPOINT;
var action_opsgenie_method = process.env.ACTION_OPSGENIE_METHOD || conf.ACTION_OPSGENIE_METHOD;
var action_opsgenie_api_key = process.env.ACTION_OPSGENIE_API_KEY || conf.ACTION_OPSGENIE_API_KEY;
var action_opsgenie_message = process.env.ACTION_OPSGENIE_MESSAGE || conf.ACTION_OPSGENIE_MESSAGE;
var action_opsgenie_team = process.env.ACTION_OPSGENIE_TEAM || conf.ACTION_OPSGENIE_TEAM;
...
/**
 * Handle Ops Genie trigger
 *
 * documentation: https://www.opsgenie.com/docs/alert-api
 */
function opsGenieTrigger(){
    if(action_opsGenie){
        console.log('ops genie trigger');

        var opsGenieObj = {
            message: action_opsgenie_message,
            description: JSON.stringify(results_json.testsuites),
            responders: [{
                    'name': action_opsgenie_team,
                    'type': 'team'
            }],
            details: results_json.testsuites.testsuite[0].$
        };

        var headers = {
            'Content-Type': 'application/json',
            'Authorization': 'GenieKey ' + action_opsgenie_api_key
        }


        var dataString = JSON.stringify(opsGenieObj);

        triggerEmail.httpSendHeaders(action_opsgenie_protocol, action_opsgenie_host, action_opsgenie_endpoint, action_opsgenie_method, headers, dataString);
    }
}
